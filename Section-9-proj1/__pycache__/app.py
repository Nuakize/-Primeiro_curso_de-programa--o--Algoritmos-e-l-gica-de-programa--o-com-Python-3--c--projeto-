...
def jogar(palavra):
    ...
    while not advinhou and tentativas > 0:
        ...
        # Verificar se a auternativa é uma única letra
        if len(tentativa) == 1 and tentativa.isalpha():
            ...
            # Quando a letra está na palavra
                ...
                print("Você acerteu a letra %s está na palavra" % tentativa)
                letras_utilizadas.append(tentativa)
    ...
    if  advinhou:
        print("Parabens! Você advinhou a palavra")
    else:
        print("Acabaram as tentativas, a palavra era %s" % palavra)

# Status do jogo
def exibir_forca(tentativas):
    estagios = [ # Fim do jogo
              """
                  --------
                  |      |
                  |      O
...
def iniciar():
    ...
    jogar(palavra)
    # Quando acabar o jogo, verifica se o usuário quer jogar novamente
